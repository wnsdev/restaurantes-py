...
class Restaurante:
    ...
    @property
    def exibir_carpio(self):
        print(f'Cardapio do Restaurante {self._nome}\n')
        for indice, item in enumerate(self._cardapio, start=1):
            mensagem = []
            for atributo in vars(item):
                nomeDoAtributo = str(atributo).replace('_', '')
                mensagem.append(f'{nomeDoAtributo.title()}: {str(vars(item)[atributo]).ljust(25).title()}')
            print(f'{indice}. {"| ".join(mensagem)}')
